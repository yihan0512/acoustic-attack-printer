...
class BiLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, device_id='0'):
    ...
    def forward(self, x, batch_size): #
        # Set initial states
        h0 = torch.zeros(self.num_layers*2, batch_size, self.hidden_size).to(torch.device('cuda:'+self.device_id if torch.cuda.is_available() else 'cpu')) # 2 for bidirection 
        c0 = torch.zeros(self.num_layers*2, batch_size, self.hidden_size).to(torch.device('cuda:'+self.device_id if torch.cuda.is_available() else 'cpu'))
